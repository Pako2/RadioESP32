...
def after_build(source, target, env):
	shutil.copy(firmware_source, 'bin/noota4mb/firmware_%s_%s.bin' % (kind, version))

#def after_parts_build(source, target, env):
#	shutil.copy(partitions_source, 'bin/noota4mb/partitions_%s_%s.bin' % (kind, version))

def after_fs_build(source, target, env):
	shutil.copy(littlefs_source, 'bin/noota4mb/littlefs_%s_%s.bin' % (kind, version))
