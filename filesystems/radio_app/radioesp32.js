...
function jumptoupman()
{
  sendMessage("{\"command\":\"jumptoupman\"}");
}
...
function commit() {
  sendMessage("{\"command\":\"config_req\"}");
	config_req = true;
	//inProgress("commit");
}


function handleAutorec(val)
{
  if (val)
  {
		$("#attemptscount").slideDown();
		$("#attemptscount").show();    
  }
  else
  {
		$("#attemptscount").slideUp();
  }
}
...
function listbt() {
	...
	$('#btcount').val(config.bluetooth.btcount);
	$('#btauto'+config.bluetooth.btauto.toString()).prop('checked', true);
	$('#btaction'+config.bluetooth.btaction.toString()).prop('checked', true);
	handleAutorec(config.bluetooth.btauto);
}

function savebt()
{
  config.bluetooth.btname = $('#btname').val();
  config.bluetooth.btauto = parseInt($(':radio[name="btauto"]:checked').val());
  config.bluetooth.btaction = parseInt($(':radio[name="btaction"]:checked').val());
  config.bluetooth.btcount = parseInt($('#btcount').val());
  uncommited();
}
...
//        break;
      case "binaries":
        ...
        break;
      case "jump":
        getContent("#jump"+obj.target.toString()+"content");
        break;
      case "config_req":
        if (config_req)
        {
...
$("#checkupd").click(function() {
  $("li.active").removeClass("active");
  $("#checkupd").parent().addClass("active");
